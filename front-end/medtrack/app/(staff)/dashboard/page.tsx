...
export default function Dashboard() {
  ...
  const handleExport = () => {
    ...
    doc.text("MedTrack Pharmacy Report", 14, 25);
    
    doc.setFontSize(10);
    doc.setTextColor(100);
    doc.text(`Generated by: ${isAdmin ? "Administrator" : "Pharmacist"}`, 14, 32);
    doc.text(`Report Date: ${dateStr}`, 14, 37);
    ...
    doc.text("Dashboard Key Metrics", 14, 55);
    
    const summaryData = [
      ["Available Medicine Variations", currentStats.available.toString()],
      ["Low Stock Alert Items", currentStats.lowStock.toString()],
      ["Medicines Expiring Soon (30 Days)", currentStats.expiringSoon.toString()],
      ["Total Business Revenue", `Rs. ${currentStats.totalRevenue.toLocaleString()}`],
      ["Total Units Sold", currentStats.totalItemsSold.toString()],
      ["Total Completed Orders", currentStats.totalOrders.toString()]
    ];
    ...
    // Low Stock Table
    const lowStockMeds = medicines.filter(m => m.quantity <= 60);
    if (lowStockMeds.length > 0) {
      doc.setFontSize(16);
      doc.text("Inventory Alert: Low Stock Items", 14, (doc as any).lastAutoTable.finalY + 20);
      
      autoTable(doc, {
        startY: (doc as any).lastAutoTable.finalY + 25,
        head: [["Medicine Name", "Category", "Batch ID", "Current Stock", "Expiry Date"]],
        body: lowStockMeds.map(m => [
          m.medicineName, 
          m.category || "N/A", 
          m.batchNumber || "N/A",
          `${m.quantity} units`, 
        ...
        headStyles: { fillColor: [220, 38, 38], textColor: 255 }, // Red 600
        alternateRowStyles: { fillColor: [254, 242, 242] } // Red 50
      });
    }

    // Recent Activity on new page
    if (activities.length > 0) {
      doc.addPage();
      doc.setFontSize(18);
      doc.text("Recent System Activities", 14, 25);
      
      autoTable(doc, {
        startY: 32,
        head: [["Activity Type", "Description", "Time Logged"]],
    ...
    for (let i = 1; i <= pageCount; i++) {
      ...
      doc.text(
        `MedTrack Inventory Management System - Confidential Report - Page ${i} of ${pageCount}`,
    ...
    doc.save(`MedTrack_Detailed_Report_${new Date().toISOString().split('T')[0]}.pdf`);
  };
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <div className="flex items-center gap-3">
          ...
          <button 
            ...
            className="flex items-center gap-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl px-4 py-2.5 text-sm font-semibold shadow-md shadow-indigo-100 transition-all active:scale-95"
          >
